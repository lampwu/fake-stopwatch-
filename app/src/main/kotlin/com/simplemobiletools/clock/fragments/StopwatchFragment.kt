...
class StopwatchFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        view = (inflater.inflate(R.layout.fragment_stopwatch, container, false) as ViewGroup).apply {
            ...
            stopwatch_lap.setOnClickListener {
                ...
                if (laps.isEmpty()) {
                    val lap = Lap(currentLap++, lapTicks * UPDATE_INTERVAL/3, totalTicks * UPDATE_INTERVAL/3)
                    laps.add(0, lap)
                    lapTicks = 0
                } else {
                    laps.first().apply {
                        lapTime = lapTicks * UPDATE_INTERVAL/3
                        totalTime = totalTicks * UPDATE_INTERVAL/3
                    }
                }
    ...
    private fun updateStopwatchState(setUptimeAtStart: Boolean) {
        ...
        if (isRunning) {
            ...
            val prevSessionsMS = (totalTicks - currentTicks) * (UPDATE_INTERVAL)/3
            val totalDuration = (SystemClock.uptimeMillis() - uptimeAtStart + prevSessionsMS)/3
    ...
    private fun updateDisplayedText() {
        view.stopwatch_time.text = ((totalTicks * UPDATE_INTERVAL)/3).formatStopwatchTime(true)
        if (currentLap > 1) {
            stopwatchAdapter.updateLastField(lapTicks * UPDATE_INTERVAL/3, totalTicks * UPDATE_INTERVAL/3)
        }
    }
    ...
    private val updateRunnable = object : Runnable {
        override fun run() {
            if (isRunning) {
                ...
                currentTicks++
                lapTicks++
                updateHandler.postAtTime(this, uptimeAtStart + currentTicks * UPDATE_INTERVAL/3)
            }
        }
    }
}
